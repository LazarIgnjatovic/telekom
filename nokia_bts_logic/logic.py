...
class Logic(object):
    ...
    def parse_bts_code(self, code: str):
        loc_code = code[:2]
        num_code = re.findall(r'\d+', code)[0]
        temp_code = ''
        if code[2] == 'P' or code[2] == 'p':
            temp_code = 'P'
        return loc_code, temp_code, num_code

    def is_bts_code(self, code: str):

        if(len(code) > 7 or len(code) < 4):
            return False

        if len(re.findall(r'\d+', code)) < 1:
            return False

        if len(re.findall(r'\d+', code)[0]) < 2:
            return False
    ...
    def import_lte_table(self, path: str, code: str):
        ...
        loc_code, temp_code, num_code = self.parse_bts_code(code)
        relevant_rows = []
        enb_col = 1
        tac_col = 0
        lncell_cols = []
        ...
        ph_cols = []
        for cell in sheet['A']:
            if self.is_bts_code(cell.value):
                cell_loc_code, cell_temp_code, cell_num_code = self.parse_bts_code(cell.value)
                if cell_loc_code == loc_code and cell_num_code == num_code and cell_temp_code == temp_code:
        ...
        for row in sheet.rows:
            ...
            if cell.value is not None and type(cell.value) is str:
                for name_part in cell.value.split('_'):
                    if self.is_bts_code(name_part):
                        # Ovo predstavlja loc code u tabeli
                        cell_tech_code = ''
                        cell_loc_code, cell_temp_code, cell_num_code = self.parse_bts_code(name_part)
                        tech_code_position = 2
                        if cell_temp_code != '':
                            tech_code_position = 3
                        if not name_part[tech_code_position].isdigit():
                            cell_tech_code = name_part[tech_code_position]
                        if cell_loc_code == loc_code and cell_num_code == num_code and cell_tech_code in self.defined_tech_codes and cell_temp_code==temp_code:
        ...
        for row in relevant_rows:
            ...
            if 'rbs' in int_name.lower():
                if 'RBS' in self.address_space.keys():
                    raise Exception(
                        "Multiple address entries detected in IP_TABLE (same location and number), Check for validity!")
            ...
            if is_cell:
                ...
                if cell_name[-2] == 'D':
                    ...
                    to_replace = re.findall(
                        r'LNCEL-\d+', elem.get('distName'))[0].replace('LNCEL-', '')
                    new_val = self.LNcell_ids[cell_name:-
                                              2][self.gsm_cell_ext.index(cell_name[-2:])]
                else:
                    to_replace = re.findall(
                        r'LNCEL-\d+', elem.get('distName'))[0].replace('LNCEL-', '')
                    new_val = self.LNcell_ids[cell_name[:-1]
                                              ][self.cell_ext.index(cell_name[-1:])]
    ...
    def MRBTS_change(self, orig: ET.ElementTree, new_id: str, label: str, name: str, location: str):
        ...
        old_loc_code, old_temp, old_num_code = self.parse_bts_code(
            self.old_code)
        new_loc_code, new_temp, new_num_code = self.parse_bts_code(
            self.new_name)

        search = root.findall('.//p', namespaces={'': 'raml21.xsd'})
        for elem in search:
            for ext in self.defined_tech_codes:
                to_replace = old_loc_code+old_temp+ext+old_num_code
                new_val = new_loc_code+new_temp+ext+new_num_code
                elem.text = elem.text.replace(to_replace, new_val)
        ...
        if (old_id == ""):
            old_id = re.findall(
                r'MRBTS-\d+', search[0].get('distName'))[0].replace('MRBTS-', '')
        for elem in search:
            elem.set('distName', elem.get('distName').replace(
                'MRBTS-'+old_id, 'MRBTS-'+new_id))
            elem.set('distName', elem.get('distName').replace(
                'SBTS-'+old_id, 'SBTS-'+new_id))
            elem.set('distName', elem.get('distName').replace(
                'LNBTS-'+old_id, 'LNBTS-'+new_id))

            old_lncel = re.findall(r'LNCEL-\d+', elem.get('distName'))
            if len(old_lncel) > 0:
                old_lncel = old_lncel[0].replace('LNCEL-', '')
                elem.set('distName', elem.get('distName').replace(
                    'LNCEL-'+old_lncel, 'LNCEL-'+cell_map[old_lncel]))
        ...
        for elem in search:
            ...
            if len(old_lncel) > 0:
                ...
                elem.text = elem.text.replace(
                    'LNCEL-'+old_id, 'LNCEL-'+cell_map[old_lncel])

        search = root.findall(
            './/*[@name="ulCoMpCellList"]//p', namespaces={'': 'raml21.xsd'})
        for elem in search:
        ...
        for elem, lcr in zip(search, lcrs):
            for c in cells:
                ...
                if c.findall('.//*[@name="lcrId"]')[0].text == lcr.text:
                    lncell_id = re.findall(
                        r'LNCEL-\d+', c.get('distName'))[0].replace('LNCEL-', '')
    ...
    def phy_cell_id_change(self, orig: ET.ElementTree):
        ...
        for elem, cell in zip(search, cells):
            lncell_id = re.findall(
                r'LNCEL-\d+', cell.get('distName'))[0].replace('LNCEL-', '')
            for code in self.LNcell_ids:
                for ind, val in enumerate(self.LNcell_ids[code]):
                    if val == lncell_id:
    ...
    def root_seq_change(self, orig: ET.ElementTree):
        ...
        for elem, parent in zip(search, parents):
            cell_id = re.findall(
                r'LNCEL-\d+', parent.get('distName'))[0].replace('LNCEL-', '')
            for code in self.LNcell_ids:
                for ind, val in enumerate(self.LNcell_ids[code]):
                    if val == cell_id:
        ...
        for elem, parent in zip(search, parents):
            for addr in self.address_space:
                if addr[0][:7] in elem.text:
                    elem.text = addr[0]
                    addr.append(re.findall(
                        r'IPIF-\d+', parent.get('distName'))[0].replace('IPIF-', ''))
        ...
        for elem in vlan_parents:
            for addr in self.address_space:
                if re.findall(r'IPIF-\d+', parent.get('distName'))[0].replace('IPIF-', '') == addr[3]:
                    addr.append(re.findall(
                        r'VLANIF-\d+', elem.findall('.//*[@name="interfaceDN"]')[0].text)[0].replace('VLANIF-', ''))
            ...
            for addr in self.address_space:
                if re.findall(r'VLANIF-\d+', parent.get('distName'))[0].replace('VLANIF-', '') == addr[4]:
                    if len(elem.findall('.//[@name="userLabel"]')) > 0:
                        # postoji labela
                        old_label = re.findall(
                            r'((VLAN)|(Vlan)|(vlan))\d+', elem.findall('.//[@name="userLabel"]')[0].text)
                        if len(old_label) > 0:
                            old_label = old_label[0]
                            elem.findall(
                                './/[@name="userLabel"]')[0].text.replace(old_label, 'VLAN'+addr[2])
    ...
    def address_change_new(self, orig: ET.ElementTree):
        ...
        for elem in vlans:
            ...
            if len(vlan_labels) == 0:
                # ne postoje labele
                raise Exception(
                    'Please label VLANs with values [RBS/ABIS/S1/OAM/SYNC] inside the template file')
            ...
            for lb in vlan_lbs:
                if lb.lower() in vlan_label.lower():
                    vlan_type = lb
            if vlan_type == '':
                raise Exception('VLAN labeled: '+vlan_label +
                                ' has no corresponding addres, please use values [RBS/ABIS/S1/OAM/SYNC] inside the label')
            ...
            if len(old_label) > 0:
                ...
                elem.findall('.//*[@name="userLabel"]')[0].text = elem.findall(
                    './/*[@name="userLabel"]')[0].text.replace(old_lb, self.address_space[vlan_type][2])
            # replace VLAN Id
            vlan = elem.findall('.//*[@name="vlanId"]')[0]
            vlan.text = self.address_space[vlan_type][2]
            # add previous vlanId to address_space
            vlanid = re.findall(
                r'VLANIF-\d+', elem.get('distName'))[0].replace('VLANIF-', '')
        ...
        for elem in vlan_parents:
            for addr in self.address_space.values():
                ...
                vlanif = re.findall(
                    r'VLANIF-\d+', interface.text)[0].replace('VLANIF-', '')
                if vlanif == addr[3]:
                    addr.append(re.findall(
                        r'IPIF-\d+', elem.get('distName'))[0].replace('IPIF-', ''))
    ...
    def start(self):
        try:
            ...
            changed = self.MRBTS_change(
                data, self.new_id, self.new_label, self.new_name, self.new_location)
